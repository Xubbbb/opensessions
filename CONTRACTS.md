...
- Config directories: `~/.claude`, the server's `$CLAUDE_CONFIG_DIR`, every `~/.claude*` sibling holding Claude state, and any directory listed in `claudeConfigDirs` in `config.json` (accounts kept elsewhere). Each account's sessions are matched to panes independently, so a `claude` started with any `CLAUDE_CONFIG_DIR` shows up as long as its directory is among these.
