...
use opensessions_runtime::project_dir_session::{
    DirSessionMap, build_dir_session_map, resolve_session_for_project_dir_preferring,
};
use opensessions_runtime::protocol::{
    AgentEvent, AgentLiveness, AgentPanelScope, AgentStatus, MetadataTone, ProtocolHello,
    ServerMessage, SessionFilterMode,
};
...
use opensessions_runtime::tmux_provider::{StdCommandRunner, TmuxProvider, agent_panes_from};
use opensessions_runtime::tracker::{AgentTracker, RegistryInput, Source};
use opensessions_runtime::transcript_tail::TailCache;
...
/// Release version, taken from `package.json` at build time (see `build.rs`).
pub const SERVER_VERSION: &str = env!("OPENSESSIONS_VERSION");
pub const PROTOCOL_VERSION: u16 = 1;
pub const QUIT_JSON: &str = r#"{"type":"quit"}"#;

/// First frame sent to every websocket client.
pub fn hello_json() -> &'static str {
    static HELLO: OnceLock<String> = OnceLock::new();
    HELLO.get_or_init(|| {
        serde_json::to_string(&ServerMessage::Hello(ProtocolHello {
...
const HTTP_READ_TIMEOUT_MS: u64 = 5_000;
...
fn debug_log(line: impl AsRef<str>) {
    ...
    let tag = TAG.get_or_init(|| format!("server pid={}", process::id()));
...
pub trait StateSource: Send + Sync + 'static {
    ...
    /// Returns a fresh state payload when the deletion changed sidebar state.
...
impl PortCommandRunner for SystemPortCommandRunner {
    ...
    fn lsof_fields(&self) -> String {
        // `lsof` is in /usr/sbin on macOS but /usr/bin on most Linux
        // distributions, so resolve it through PATH first and only then fall
        // back to the known absolute locations.
        for binary in LSOF_CANDIDATES {
            let Ok(output) = process::Command::new(binary)
...
/// A live registry-backed Claude Code session and where its transcript is.
...
pub fn default_state_source_from_env(
    ...
    if env("TMUX").is_some() {
        ...
        let config = home.as_deref().map(load_config_from_home);
        ...
        // Only string theme names are wired end to end; an inline theme
        // object is parsed for compatibility but not applied.
        if let Some(theme) = config
            .as_ref()
            .and_then(|config| config.theme.as_ref())
            .and_then(Value::as_str)
        {
            source = source.with_theme(theme.to_string());
        }
...
impl ReadOnlyMuxStateSource {
    ...
    pub fn with_session_filter(self, filter: SessionFilterMode) -> Self {
    ...
    fn persist_config(&self, what: &str, updates: OpensessionsConfig) {
        let Some(home) = std::env::var_os("HOME").map(PathBuf::from) else {
            debug_log(format!("{what}: skipped config save because HOME is unset"));
            return;
        };
        if let Err(err) = save_config_to_home(&home, updates) {
            debug_log(format!("{what}: failed to save config: {err}"));
        }
    }
    ...
    /// Refresh the transcripts of live registry sessions and hand their tail
    /// state to the tracker. Returns whether any row changed.
    fn enrich_claude_registry_rows(&self, tails: &mut TranscriptTails) -> bool {
        let threads = self.claude_registry_threads.lock().unwrap().clone();
        let mut changed = false;
        let mut hints = Vec::new();
        for thread in &threads {
            let Some(path) = tails.claude_transcript_path(thread) else {
                continue;
            };
            let Some(tail) = tails.claude.refresh(&path) else {
                continue;
            };
            if tail.changed || !tails.hinted.contains(&thread.session_id) {
                hints.push((thread.session_id.clone(), tail.state.hint()));
            }
        }
        if !hints.is_empty() {
            let mut tracker = self.agent_tracker.lock().unwrap();
            for (session_id, hint) in hints {
                changed = tracker.apply_transcript_hint(&session_id, hint) || changed;
                tails.hinted.insert(session_id);
            }
        }
        let live = threads
            .iter()
            ...
            .collect::<HashSet<_>>();
        tails.hinted.retain(|session_id| live.contains(session_id));
        tails
            .claude_paths
    ...
    /// Parse a hook/script context body, resolving a tmux session id (`$N`,
    /// which the hooks send because ids are shell-safe) to the session name.
    fn parse_hook_context(&self, body: &str) -> Option<HttpContext> {
        let mut context = parse_context(body)?;
        if context.session.starts_with('$') {
            context.session = self
                .providers
                .iter()
                .find_map(|provider| provider.resolve_session_id(&context.session))?;
        }
        Some(context)
    }
...
impl StateSource for ReadOnlyMuxStateSource {
    ...
    fn cleanup_mux_hooks(&self) {
        for provider in &self.providers {
            // Clients were told to quit; remove their panes ourselves rather
            // than relying on `pane-died` hooks that are about to be removed
            // and that tmux may never fire (see `nudge_dead_panes`).
            // `hide_sidebar` also releases the window's remain-on-exit, which
            // must happen while the sidebar pane still identifies the window.
        ...
        match command.get("type").and_then(Value::as_str)? {
            ...
            "set-theme" => {
                ...
                self.persist_config(
                    "set-theme",
                    OpensessionsConfig {
                        theme: Some(Value::String(theme.clone())),
                        ..OpensessionsConfig::default()
                    },
                );
                *self.theme.lock().unwrap() = Some(theme);
            ...
            "mark-seen" => {
                let name = command.get("name")?.as_str()?;
                let changed = self.agent_tracker.lock().unwrap().mark_seen(name);
                changed.then(|| self.snapshot_json())
            }
            _ => None,
        }
    }
    ...
    fn handle_pi_runtime_delete(&self, body: &Value) -> Result<Option<String>, PiRuntimeError> {
        ...
        let Some(info) = self.pi_runtime_registry.lock().unwrap().delete(pid) else {
            return Ok(None);
        };
        // A clean pi shutdown is the authoritative "this agent is closed"
        // signal, so drop its tracker entry instead of waiting for pruning.
        let dismissed = self
            .resolve_session_for_project_dir(&info.cwd)
            .is_some_and(|session| {
                self.agent_tracker
                    .lock()
                    .unwrap()
    ...
    fn handle_http_hook(&self, path: &str, body: &str) -> Option<String> {
        match path {
            ...
            "/pane-exited" | "/pane-died" => {
                // One snapshot for the whole sweep: computing the display
                // order per sidebar pane would block the event loop for
                // hundreds of milliseconds and starve other hook requests.
                ...
                let fallback_sessions = self
                    ...
                    .filter_map(|pane| {
                        let fallback = neighbour_session(&display_names, &pane.session_name)?;
                ...
                // A pane just went away: re-sync agent pane presence now and
                // push the result, rather than leaving the sidebar showing the
                // closed agent until the next poll tick.
                Some(self.snapshot_json())
            }
...
impl ReadOnlyMuxStateSource {
    ...
    fn apply_agent_watcher_snapshot(
        &self,
    ...
    /// Attribute a transcript snapshot to a session by its directory; a tie
    /// between sessions at the same directory goes to the one with a live
    /// pane of that agent.
    fn resolve_agent_watcher_session(
        &self,
        snapshot: &AgentWatcherSnapshot,
        dir_session_map: &DirSessionMap,
        panes: &[MuxPane],
        ...
        let preferred = sessions_with_agent_pane(panes, snapshot.agent);
        resolve_session_for_project_dir_preferring(project_dir, dir_session_map, &preferred)
    }

    fn dir_session_map(&self) -> DirSessionMap {
    ...
    /// Resolve a project/worktree directory to a session across providers.
    fn resolve_session_for_project_dir(&self, project_dir: &str) -> Option<String> {
        resolve_session_for_project_dir_preferring(
            project_dir,
            &self.dir_session_map(),
            &HashSet::new(),
        )
    }
    ...
    fn resolve_agent_event_session(&self, body: &Value) -> Option<String> {
        ...
        if let Some(project_dir) = body.get("projectDir").and_then(Value::as_str) {
            ...
            let agent = body
                ...
                .and_then(Value::as_str)
                .unwrap_or_default();
            let (panes, _) = self.observe_mux();
            let preferred = sessions_with_agent_pane(&panes, agent);
            if let Some(session) = resolve_session_for_project_dir_preferring(
                project_dir,
                &dir_session_map,
                &preferred,
            ) {
    ...
    fn ensure_sidebar(&self, body: &str) -> bool {
        ...
        for provider in &self.providers {
            ...
            // A client parked in the stash session must not grow sidebars there.
            if session_name == "_os_stash" {
                continue;
            }
            if provider
...
/// The session a client should land in when `name` goes away: the previous
/// one in display order, else the next.
fn neighbour_session(names: &[String], name: &str) -> Option<String> {
    session_before_in(names, name).or_else(|| session_after_in(names, name))
}

fn session_order_path(home: &Path) -> PathBuf {
    home.join(".config")
        .join("opensessions")
...
async fn run_tmux_state_poll_loop(
    ...
    interval.set_missed_tick_behavior(MissedTickBehavior::Skip);
    // The tmux server this process hooked into. If it exits (or a new one
    // starts on the same socket) the hooks are gone with it and this server
    // would otherwise linger, be reused by the plugin because its port still
    // answers, and leave the new tmux server without any hooks.
    let mux_ids = source
        .providers
        .iter()
        .map(|provider| provider.mux_server_id())
        .collect::<Vec<_>>();
    let mut mux_missing_ticks = 0_u32;
    ...
    loop {
        tokio::select! {
            _ = shutdown_rx.recv() => return,
            _ = interval.tick() => {
                let mux_changed = source
                    .providers
                    .iter()
                    .zip(&mux_ids)
                    .any(|(provider, original)| original.is_some() && provider.mux_server_id() != *original);
                if mux_changed {
                    mux_missing_ticks += 1;
                    if mux_missing_ticks >= MUX_GONE_TICKS {
                        debug_log("tmux_state_poll_loop: tmux server gone or replaced, shutting down");
                        let _ = shutdown.send(());
                        return;
                    }
                    continue;
                ...
                for provider in &source.providers {
                    if provider.nudge_dead_panes() {
                        debug_log("tmux_state_poll_loop: nudged tmux to reap a dead pane");
                    }
                }
        ...
        tokio::select! {
            ...
            _ = interval.tick() => {
                ...
                if transcript_tick {
                    ...
                    .await
                    .unwrap_or_default();
                    let mut scan_tails = scan_tails;
                    changed = source.enrich_claude_registry_rows(&mut scan_tails) || changed;
                    tails = Some(scan_tails);
                    debug_log(format!(
                        "agent_watcher_loop: tick scanned {} snapshots",
                        snapshots.len()
                    ));
                    let dir_session_map = source.dir_session_map();
                    ...
                    for snapshot in snapshots {
                        ...
                        if source.apply_agent_watcher_snapshot(snapshot, &dir_session_map, &panes) {
                            debug_log(format!(
...
/// Per-file parse state kept between transcript ticks so large transcripts
...
struct TranscriptTails {
    ...
    /// (len, mtime) so an unchanged file is not parsed again.
    amp: HashMap<PathBuf, (u64, u64, Option<AgentWatcherSnapshot>)>,
    /// Where each live registry session's transcript was found.
    claude_paths: HashMap<String, PathBuf>,
    /// Registry sessions whose transcript hint has been delivered at least
    /// once.
    hinted: HashSet<String>,
}

impl TranscriptTails {
    /// The transcript of a registry session: under the project folder named
    /// after its cwd, or wherever a file with its id lives under that config
    /// directory (the cwd may have been recorded differently).
    fn claude_transcript_path(&mut self, thread: &RegistryThread) -> Option<PathBuf> {
        if let Some(path) = self.claude_paths.get(&thread.session_id)
            && path.is_file()
        {
            return Some(path.clone());
        }
        let file_name = format!("{}.jsonl", thread.session_id);
        let projects = thread.config_dir.join("projects");
        let candidate = thread
            .cwd
            .as_deref()
            .map(|cwd| {
                projects
                    .join(encode_agent_project_dir(cwd))
                    .join(&file_name)
            })
        ...
        self.claude_paths
            .insert(thread.session_id.clone(), candidate.clone());
        Some(candidate)
    }

    fn forget_stale(&mut self, now_ms: u64) {
...
/// Sessions that show a live pane of `agent`, used to settle same-directory
/// attribution ties.
fn sessions_with_agent_pane(panes: &[MuxPane], agent: &str) -> HashSet<String> {
    let by_pane = panes
        .iter()
        .map(|pane| (pane.pane_id.as_str(), pane.session_name.as_str()))
        .collect::<HashMap<_, _>>();
    agent_panes_from(panes)
        .into_iter()
        .filter(|pane| pane.agent == agent)
        .filter_map(|pane| {
            by_pane
...
fn scan_amp_threads(
    ...
    for entry in entries.flatten() {
        ...
        let Some((len, mtime_ms)) = file_len_and_mtime_ms(&path) else {
            continue;
        };
        if now_ms.saturating_sub(mtime_ms) > AGENT_WATCHER_RECENT_MS {
            continue;
        }
        let snapshot = match tails.amp.get(&path) {
            Some((cached_len, cached_mtime, snapshot))
                if *cached_len == len && *cached_mtime == mtime_ms =>
            {
                snapshot.clone()
            }
            _ => {
                ...
                let snapshot = amp_snapshot_from_thread_json(&raw, mtime_ms);
                tails
                    .amp
                    .insert(path.clone(), (len, mtime_ms, snapshot.clone()));
                snapshot
            }
        };
...
fn scan_codex_sessions(
    ...
    for path in collect_jsonl_files(&sessions_dir) {
        ...
        let Some(tail) = tails.codex.refresh(&path) else {
            continue;
        };
        if let Some(snapshot) = tail.state.snapshot(
            &thread_id,
...
fn run_process_with_timeout(
    ...
    let mut child = command
        ...
        .spawn()
        .ok()?;
    // Drain the pipes on helper threads: a child that writes more than the
    // pipe buffer (64 KiB on Linux) blocks until someone reads, and polling
    // `try_wait` alone would only ever see it "still running" until the
    // timeout killed it.
...
fn file_mtime_ms(path: &Path) -> Option<u64> {
    file_len_and_mtime_ms(path).map(|(_, mtime)| mtime)
}

fn file_len_and_mtime_ms(path: &Path) -> Option<(u64, u64)> {
...
impl ServerHandle {
    ...
    /// A handle that can request shutdown from another task, e.g. a signal
    /// listener, while the caller waits in [`ServerHandle::wait_shutdown`].
...
async fn handle_connection(
    ...
    let mut request = tokio::time::timeout(
        Duration::from_millis(HTTP_READ_TIMEOUT_MS),
        read_http_header(&mut stream),
    )
    .await
    .map_err(|_| ServerError::new("timed out reading http header"))??;
    let parsed = parse_http_request(&request)?;
    let content_length = parsed.content_length();
    if content_length > MAX_HTTP_BODY_BYTES {
        // Every body this server accepts is a few KB of JSON or a hook
        // context line; refuse anything larger before allocating for it.
        stream
            .write_all(
    ...
    .await
    .map_err(|_| ServerError::new("timed out reading http body"))??;
    if !parsed.is_websocket_upgrade() {
        debug_log(format!(
            "http: {} {} body={:?}",
            parsed.method,
            parsed.path,
            String::from_utf8_lossy(http_body(&request)).trim(),
        ));
    }
...
async fn read_remaining_http_body(
    ...
    let total = start_len
        ...
        .ok_or_else(|| ServerError::new("content-length overflow"))?;
...
mod tests {
    ...
    fn hello_frame_carries_the_package_version_and_protocol() {
        ...
        assert!(
            package_json.contains(&format!("\"version\": \"{SERVER_VERSION}\"")),
            "SERVER_VERSION {SERVER_VERSION} must come from package.json"
        );
        assert_eq!(
            hello_json(),
            format!(
                r#"{{"type":"hello","protocol":{PROTOCOL_VERSION},"serverVersion":"{SERVER_VERSION}"}}"#
            )
        );
    }
}
