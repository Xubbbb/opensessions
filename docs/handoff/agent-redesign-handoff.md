...
- Sidebar selection is sticky (`packages/sidebar-core-rs/src/app.rs`): `ActivateSession` (the client returning to the sidebar's session) no longer rehomes the selected row to the local session, and a pending switch that landed elsewhere no longer does either; the selection moves only on user input or when its row disappears. Upstream's "stale temporary focus" E2E was replaced by `tmux_sidebar_keeps_the_selected_row_when_the_client_returns_to_the_session`. Server-driven focus moves are logged as `[sidebar-core] focus moved by server message …` when `OPENSESSIONS_DEBUG_LOG` is set.
