...
3. **Built-in agent watchers**: the Rust server scans Amp, Claude Code, Codex, OpenCode, Pi, and Droid state directly and converts it into `AgentEvent`s. For Claude Code the only row source is its session registry (`<config>/sessions/<pid>.json`: session id, pane, status, name — see `packages/runtime-rs/src/claude_registry.rs`); transcripts only enrich registry rows and never create rows (Claude Code older than 2.1, which has no registry, is not supported).
